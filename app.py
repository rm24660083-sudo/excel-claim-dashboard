...
if uploaded_file:
    ...
    if "MonthKey" in df.columns:
        ...
        # -----------------------------
    # Top Defect by Supplier (แท่งใหญ่ขึ้นชัดเจน)
    # -----------------------------
    ...
    if "SUP" in df.columns and "Defect" in df.columns:
        ...
        fig4.update_layout(
            bargap=0.05,         # ลดช่องว่างระหว่างแท่ง (แท่งหนาขึ้น)
            bargroupgap=0.02,    # ลดช่องว่างระหว่างกลุ่ม
            height=900           # เพิ่มความสูงของกราฟ
        )

        # เพิ่มเส้นขอบแท่งให้ดูหนาและชัด
        fig4.update_traces(marker_line_width=1.5)
